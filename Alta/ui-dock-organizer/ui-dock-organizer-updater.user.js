// ==UserScript==
// @name         Cross-Origin UI Dock Organizer Updater
// @namespace    homebot.ui-dock-organizer.updater
// @version      0.1.1
// @description  Loads and auto-updates the Cross-Origin UI Dock Organizer script from GitHub.
// @author       OpenAI
...
// @updateURL    https://raw.githubusercontent.com/ugomez809/TamperMonkey-Automatic-Quote-New/main/Alta/ui-dock-organizer/ui-dock-organizer-updater.user.js
// @downloadURL  https://raw.githubusercontent.com/ugomez809/TamperMonkey-Automatic-Quote-New/main/Alta/ui-dock-organizer/ui-dock-organizer-updater.user.js
// ==/UserScript==
(function () {
  'use strict';

  var LOADER_VERSION = '0.1.1';
  var TARGET_ID = "ui-dock-organizer";
  var TARGET_LABEL = "Cross-Origin UI Dock Organizer";
  var TARGET_FILE = "ui-dock-organizer.user.js";
  var BASE_URL = "https://raw.githubusercontent.com/ugomez809/TamperMonkey-Automatic-Quote-New/main/Alta/ui-dock-organizer";
  ...
  function boot() {
    ...
    window.setInterval(function () { checkForUpdates({ runIfNoCache: false, forceReload: false }).catch(warn); }, CHECK_INTERVAL_MS);
  }
  ...
  function executeTarget(code, source) {
    ...
    try {
      ...
      if (RUN_IN_PAGE_CONTEXT) {
        var pageEval = (typeof unsafeWindow !== 'undefined' && unsafeWindow && unsafeWindow.eval) ? unsafeWindow.eval.bind(unsafeWindow) : window.eval.bind(window);
        pageEval(runnable);
        return;
      }
      eval(runnable);
    ...
    return new Promise(function (resolve, reject) {
      GM_xmlhttpRequest({
        ...
        onload: function (response) {
          ...
          if (!remote.includes('// ==UserScript==')) { reject(new Error('response did not look like a userscript')); return; }
          resolve(remote);
        },
  ...
  function registerMenu() {
    ...
    GM_registerMenuCommand('Check ' + TARGET_LABEL + ' now', function () { checkForUpdates({ runIfNoCache: !executed, forceReload: true }).catch(warn); });
    GM_registerMenuCommand('Clear ' + TARGET_LABEL + ' cache', function () { clearCache(); location.reload(); });
  }
  ...
  function extractVersion(code) { var match = String(code || '').match(/^\/\/\s*@version\s+([^\s]+)/m); return match ? match[1] : 'unknown'; }
  ...
  function storageGet(key, fallback) { try { return GM_getValue(key, fallback); } catch (err) { return fallback; } }
  function storageSet(key, value) { try { GM_setValue(key, value); } catch (err) {} }
  function storageDelete(key) { try { GM_deleteValue(key); } catch (err) {} }
  function truthy(value) { return /^(1|true|yes|y)$/i.test(String(value || '')); }
  function formatTime(value) { var ms = Number(value || 0); return ms ? new Date(ms).toLocaleString() : 'never'; }
